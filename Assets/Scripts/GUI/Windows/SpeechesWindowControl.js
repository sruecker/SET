...
private function windowFunction (windowID : int) {
	...
	if (__displayMode != BUBBLE_MODE) {
		toggleButton = __displayMode == READER_MODE ? GUIContent(toggleJumpDisabledButtonTexture, "Jumping Disabled"):GUIContent(toggleJumpButtonTexture, "Jumping Enabled");
		if (GUI.Button(Rect(winRect.width - 80, 5, 16, 16), toggleButton, dragButtonStyle)) {
			if (__displayMode == READER_MODE) __displayMode = JUMP_READER_MODE;
			else __displayMode = READER_MODE;
		}
	} else {
		GUI.enabled = false;
		GUI.Button(Rect(winRect.width - 80, 5, 16, 16), GUIContent(toggleJumpDisabledButtonTexture, "Jumping Disabled"), dragButtonStyle);
		GUI.enabled = true;
	}

	toggleButton = __displayMode == BUBBLE_MODE ? GUIContent(toggleReaderButtonTexture, "Show Reader View"):GUIContent(toggleBubbleButtonTexture, "Show Speech Bubble View");
	if (GUI.Button(Rect(winRect.width - 60, 5, 16, 16), toggleButton, dragButtonStyle)) {
		if (__displayMode == BUBBLE_MODE) __displayMode = JUMP_READER_MODE;
		else __displayMode = BUBBLE_MODE;
	}
	
	if (GUI.Button(Rect(winRect.width - 40, 5, 16, 16), GUIContent(popOutButtonTexture, "Show Floating Reader"), dragButtonStyle)) {
	...
	if (GUI.Button(Rect(winRect.width - 20, 5, 16, 16), undockingTexture, dragButtonStyle)) {
...
private function windowPopOutFunction(windowID: int) {
	...
	var toggleButton:GUIContent = __popOutDisplayMode == READER_MODE ? GUIContent(toggleJumpDisabledButtonTexture, "Jumping Disabled"):GUIContent(toggleJumpButtonTexture, "Jumping Enabled");
	if (GUI.Button(Rect(winRect.width - 60, 5, 16, 16), toggleButton, dragButtonStyle)) {
		if (__popOutDisplayMode == READER_MODE) __popOutDisplayMode = JUMP_READER_MODE;
		else __popOutDisplayMode = READER_MODE;
	}

	if (GUI.Button(Rect(winRect.width - 40, 5, 16, 16), GUIContent(copyButtonTexture, "Copy Changes"), dragButtonStyle)) {
		createSpeechPositions(true);
	}

	if (GUI.Button(Rect(winRect.width - 20, 5, 16, 16), GUIContent(closeButtonTexture, "Close Window"), dragButtonStyle)) {
		__showPopOut = false;
	}
